...
sudo apt install -qfy build-essential cmake cmake-extras curl findutils \
...
sudo apt install -qfy libswresample-dev libsystemd-dev libtiff-dev libtiffxx6 \
...
sudo apt install -qfy meson ninja-build openssl psmisc python3-mako \
...
sudo apt install -qfy libdw1t64 usb.ids i2c-tools libarchive13t64 libmalcontent-0-0 \
...
sudo apt install -qfy xdg-dbus-proxy xdg-desktop-portal-hyprland fzf bluez \
    ...
    uwsm uv waybar pavucontrol-qt libreoffice pulseaudio \
...
BKPDIR=$(mktemp -d -p ~/)
mv ~/.{bash*,profile,vimrc,ssh,gnupg,bin} $BKPDIR
